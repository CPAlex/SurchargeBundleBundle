...
use UJM\ExoBundle\Controller\ExerciseController as BaseController;
        ...
        if ($exoAdmin === true) {
            ...
            foreach ($interactions as $interaction) {
                ...
                switch($interaction->getType()) {
                    case 'InteractionQCM' :
                        $choices = $em->getRepository('UJMExoBundle:InteractionQCM')
                            ->getInteractionQCM($interaction->getId());
                        foreach ($choices as $choice) {
                            $points[$interaction->getId()] = $choice =$exerciseSer->qcmMaxScore($choices[0]);

                        }
                        break;

                    case 'InteractionMatching' :
                        $tests = $em->getRepository('UJMExoBundle:InteractionMatching')
                            ->getInteractionMatching($interaction->getId());
                        foreach ($tests as $test) {
                            $points[$interaction->getId()] = $test =$exerciseSer->matchingMaxScore($tests[0]);

                        }
                        break;
                    case 'InteractionHole'  :

                        $holes = $em->getRepository('UJMExoBundle:InteractionHole')
                            ->getInteractionHole($interaction->getId());
                        foreach ($holes as $hole) {
                            $points[$interaction->getId()] = $hole = $exerciseSer->holeMaxScore($holes[0]);

                        }
                        break;

                    case 'InteractionGraphic':
                        $graphs = $em->getRepository('UJMExoBundle:InteractionGraphic')
                            ->getInteractionGraphic($interaction->getId());
                        foreach ($graphs as $graph) {
                            $points[$interaction->getId()] = $graph = $exerciseSer->graphicMaxScore($graphs[0]);
                        }
                        break;
                    case 'InteractionOpen':
                        $opens = $em->getRepository('UJMExoBundle:InteractionOpen')
                            ->getInteractionOpen($interaction->getId());
                        foreach ($opens as $open) {
                            $points[$interaction->getId()] = $open = $exerciseSer->openMaxScore($opens[0]);
                        }
                        break;
